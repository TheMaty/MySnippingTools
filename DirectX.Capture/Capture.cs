...
//
// Future Improvements
//  - Merge DShowNET into DirectX.Capture for ease of use (rename files to InteropDsExtend.cs?, or put into folder?)
//  - reset VideoCaps/AudioCaps when rerender/derender/create/destroy
//  - try retrieve/set framesize properties directly, if fail then set "derenderOnStreamConfig" flag and in the future derender
//  - Document when settings will be lost (perhaps add MsgBoxes to CaptureTest sample)
...
//	- When rebuilding graph, try saving and restoring all properties
...
//  - Change VideoFormat (NTSC/PAL), I think you need to use IAMAnalogVideoDecoder  (WDM only)
//  - BT878 doesn't support overlay while capturing > 320x240 (http://groups.google.ca/groups?q=g:thl203370098d&dq=&hl=en&lr=&ie=UTF-8&selm=agpkio%2461n%241%40news3.bu.edu)
//
//
...
{
	/// <summary> Possible video standards </summary>
	...
	{
		[Label("56555941-0000-0010-8000-00AA00389B71")]		AYUV,		// 4:4:4 YUV format
		[Label("59565955-0000-0010-8000-00AA00389B71")]		UYVY,		// UYVY (packed 4:2:2)
		[Label("31313459-0000-0010-8000-00AA00389B71")]		Y411,		// Same as Y41P
		[Label("50313459-0000-0010-8000-00AA00389B71")]		Y41P,		// Y41P (packed 4:1:1)
		[Label("31313259-0000-0010-8000-00AA00389B71")]		Y211,		// Y211
		[Label("32595559-0000-0010-8000-00AA00389B71")]		YUV2,		// YUV2 (packed 4:2:2)
		[Label("55595659-0000-0010-8000-00AA00389B71")]		YVYU,		// YVYU (packed 4:2:2)
		[Label("56595559-0000-0010-8000-00AA00389B71")]		YUYV,		// YVYU (packed 4:2:2)
		[Label("39555659-0000-0010-8000-00AA00389B71")]		YVU9,		// YVU9
		[Label("32315659-0000-0010-8000-00AA00389B71")]		YV12,		// YV12
		[Label("e436eb78-524f-11ce-9f53-0020af0ba770")]		RGB1,		// RGB, 1 bpp, palettized
		[Label("e436eb79-524f-11ce-9f53-0020af0ba770")]		RGB4,		// RGB, 4 bpp, palettized
		[Label("e436eb7a-524f-11ce-9f53-0020af0ba770")]		RGB8,		// RGB, 8 bpp
		[Label("e436eb7b-524f-11ce-9f53-0020af0ba770")]		RGB565,		// RGB 565, 16 bpp
		[Label("e436eb7c-524f-11ce-9f53-0020af0ba770")]		RGB555,		// RGB 555, 16 bpp
		[Label("e436eb7d-524f-11ce-9f53-0020af0ba770")]		RGB24,		// RGB, 24 bpp
		[Label("e436eb7e-524f-11ce-9f53-0020af0ba770")]		RGB32,		// RGB 32 bpp, no alpha channel
		[Label("773c9ac0-3274-11d0-B724-00aa006c1A01")]		ARGB32		// RGB, 32 bpp, alpha channel
	}
	...
	public class Capture
	{
		...
		/// <summary>
		///  The capture capabilities of the video device.
		/// </summary>
		/// <remarks>
		...
		///  This property cannot be changed while capturing or cued. 
		/// </remarks> 
		public ColorSpaceEnum PreviewColorSpace
		{
		...
		///  
		///  <para>
		///  The frame rate used to capture video can be different
		///  than the frame rate used in preview.</para>
		///  
		/// <para>
		///  This property cannot be changed while capturing. Changing 
		///  this property while preview is enabled will cause some 
		...
		///  cue. Call Cue() again to re-cue the capture. </para>
		/// </remarks>
		public double PreviewFrameRate
		{
			get
			{
				long avgTimePerFrame = (long) getStreamConfigSetting( previewStreamConfig, "AvgTimePerFrame" );
				return( (double) 10000000 / avgTimePerFrame );
			}
			set
			{
				long avgTimePerFrame = (long) ( 10000000 / value );
				setStreamConfigSetting( previewStreamConfig, "AvgTimePerFrame", avgTimePerFrame );
			}
		}
		...
		/// </summary>
		/// <remarks>
		/// Set this this property to value between 0.0 to 1.0
		/// where 0.0 will give the darkest picture and 1.0 will
		/// give the lightest. For most video cards a value of
		/// 0.5 is the default.
		/// </remarks>
		public double Brightness
		{
			get { return getProcAmpProperty( VideoProcAmpProperty.Brightness ); }
			set { setProcAmpProperty( VideoProcAmpProperty.Brightness, value ); }
		}
		...
		/// <remarks>
		/// Set this this property to value between 0.0 to 1.0
		/// where 0.0 will give the no contrast
		/// picture and 1.0 will give the highest contrast. 
		/// For most video cards a value of 0.5 is the default.
		/// </remarks>
		public double Contrast
		{
			get { return getProcAmpProperty( VideoProcAmpProperty.Contrast ); }
			set { setProcAmpProperty( VideoProcAmpProperty.Contrast, value ); }
		}

		/// <summary>
		/// Gets and sets the hue of the video.
		/// </summary>
		/// <remarks>
		/// Set this this property to value between 0.0 to 1.0
		/// For most video cards a value of 0.5 is the default.
		...
		public double Hue
		{
			get { return getProcAmpProperty( VideoProcAmpProperty.Hue ); }
			set { setProcAmpProperty( VideoProcAmpProperty.Hue, value ); }
		}
		...
		/// <remarks>
		/// Set this this property to value between 0.0 to 1.0
		/// where 0.0 will give the no saturation (greyscale) 
		/// picture and 1.0 will give the highest saturation. 
		/// For most video cards a value of 0.5 is the default.
		/// </remarks>
		public double Saturation
		{
			get { return getProcAmpProperty( VideoProcAmpProperty.Saturation ); }
			set { setProcAmpProperty( VideoProcAmpProperty.Saturation, value ); }
		}
		...
		protected VideoCapabilities	videoCaps = null;					// Property Backer: capture capabilities of video device
		protected VideoCapabilities	previewCaps = null;					// Property Backer: preview capabilities of video device
		...
		protected IAMStreamConfig	previewStreamConfig = null;			// DShow Filter: configure preview frame rate, size
		protected IAMStreamConfig	audioStreamConfig = null;			// DShow Filter: configure sample rate, sample size
		protected IAMVideoProcAmp	videoProcAmp = null;				// DShow Filter: interface for setting brightness/contrast/hue/saturation
		protected IAMAnalogVideoDecoder videoDecoder = null;			// DShow Filter: interface for changin video format ( NTSC, PAL )
		...
		protected void createGraph()
		{
			...
			if ( (int)graphState < (int)GraphState.Created )
			{
				...
				// Retrieve video compression interface if available
				cat = PinCategory.Capture;
				med = MediaType.Interleaved;
				iid = typeof(IAMVideoCompression).GUID;
				...
				IAMVideoCompression vc = o as IAMVideoCompression;

				// Retrieve IAMStreamConfig for the capture pin 
				cat = PinCategory.Capture;
				med = MediaType.Interleaved;
				...
				// Retrieve IAMStreamConfig for the preview pin
				...
				// Retrieve the IAMVideoProcAmp interface
				// for setting brightness, contrast, hue, saturation
				videoProcAmp = videoDeviceFilter as IAMVideoProcAmp;
				...
				try 
				{
					/*
										// Test country code
										Debug.WriteLine("Testing country code... ");

										for (c=0; c<1000; c++)
										{
											hr = t.put_CountryCode(c);
											Debug.WriteLine("Setting country code to    "+ c +"      US...        " + (hr==0?"success":" FAILED(" + hr + ")"));
										}
					*/					

					/*
										// Retrieve the IAMVideoProcAmp interface if available
										Debug.WriteLine("");
										Debug.WriteLine("Testing brightness, contrast, hue...");
										IAMVideoProcAmp videoProcAmp;
										videoProcAmp = videoDeviceFilter as IAMVideoProcAmp;
										if (videoProcAmp == null) 
											Debug.WriteLine("Failed to get IAMVideoProc interface");
										else
										{
											Debug.Write("Found IAMVideoProcAmp interface... ");

											hr = videoProcAmp.GetRange(VideoProcAmpProperty.Brightness, out min, out max, out stepping, out def, out flags);
											Debug.WriteLine("GetRange(0): min=" + min + ", max=" + max + ", stepping=" + stepping + ", def=" + def + ", flags=" + flags + ", hr=" + hr);
											//hr = videoProcAmp.GetRange(1, out min, out max, out stepping, out def, out flags);
											//Debug.WriteLine("GetRange(1): min=" + min + ", max=" + max + ", stepping=" + stepping + ", def=" + def + ", flags=" + flags + ", hr=" + hr);
											//hr = videoProcAmp.GetRange(2, out min, out max, out stepping, out def, out flags);
											//Debug.WriteLine("GetRange(2): min=" + min + ", max=" + max + ", stepping=" + stepping + ", def=" + def + ", flags=" + flags + ", hr=" + hr);
											//hr = videoProcAmp.GetRange(3, out min, out max, out stepping, out def, out flags);
											//Debug.WriteLine("GetRange(3): min=" + min + ", max=" + max + ", stepping=" + stepping + ", def=" + def + ", flags=" + flags + ", hr=" + hr);

											//hr= videoProcAmp.Get(0, out i, out flags);
											//Debug.WriteLine("Get(0): " + (hr==0?i.ToString():"FAILED(" + hr + ")"));
											hr= videoProcAmp.Get(VideoProcAmpProperty.Contrast, out i, out flags);
											Debug.WriteLine("Get(1): " + (hr==0?i.ToString():"FAILED(" + hr + ")"));
											//hr= videoProcAmp.Get(2, out i, out flags);
											//Debug.WriteLine("Get(2): " + (hr==0?i.ToString():"FAILED(" + hr + ")"));
											//hr= videoProcAmp.Get(3, out i, out flags);
											//Debug.WriteLine("Get(3): " + (hr==0?i.ToString():"FAILED(" + hr + ")"));

											hr = videoProcAmp.Set(VideoProcAmpProperty.Brightness, 1000, 0);
											hr = videoProcAmp.Set(VideoProcAmpProperty.Contrast, 2000, 0);
											hr = videoProcAmp.Set(VideoProcAmpProperty.Hue, 3000, 1);
											hr = videoProcAmp.Set(VideoProcAmpProperty.Saturation, 4000, 2);
										}
					*/
					...
					/*
										...
										else
										{
											...
											hr = a.get_TVFormat(out i);
											Debug.WriteLine("TV Format = " + (hr==0?i.ToString():" FAILED(" + hr + ")"));

											hr = a.put_TVFormat(0x10);
											Debug.WriteLine("Setting video format to PAL_B... " + (hr==0?"success":" FAILED(" + hr + ")"));

											hr = a.get_TVFormat(out i);
											Debug.WriteLine("TV Format = " + (hr==0?i.ToString():" FAILED(" + hr + ")"));

											hr = a.put_TVFormat(0x1000);
											Debug.WriteLine("Setting video format to SECAM_B... " + (hr==0?"success":" FAILED(" + hr + ")"));
					...
					*/					
					...
					// Test setting the color space / media subtype
					//					Debug.WriteLine("");
					//					Debug.WriteLine("Testing color space...");
					
				
					/*
										// This won't work use iAMAnalogVideoDecoder (see comments at top)
										//Guid subtype = (Guid) getStreamConfigSetting(videoStreamConfig, "subType");
										//setStreamConfigSetting(videoStreamConfig, "subType", AnalogVideoStandard.PAL_B); 

										// No good - returne VideoInfoHeader instead of VideoInfoHeader2
										//long aspectX;
										//aspectX = (long) getStreamConfigSetting(videoStreamConfig, "PictAspectRatioX");
										//setStreamConfigSetting(videoStreamConfig, "PictAspectRatioX", 2);
		...
		protected void destroyGraph()
		{
			...
			// This is a copy of the video device
			videoProcAmp = null;
		...
		protected object getStreamConfigSetting( IAMStreamConfig streamConfig, string fieldName)
		{
			...
			try 
			{
				...
				{
					structType = mediaType.GetType();
					fieldInfo = structType.GetField( fieldName );
					if ( fieldInfo != null )
					{
						// Extract the field's current value
						returnValue = (object) fieldInfo.GetValue( mediaType ); 
					}
					else
		...
		private ColorSpaceEnum getMediaSubType( IAMStreamConfig streamConfig )
		{
			...
			ColorSpaceEnum retval = ColorSpaceEnum.ARGB32;
			bool found;
			IntPtr pmt = IntPtr.Zero;
			AMMediaType mediaType = new AMMediaType();
			...
			try 
			{
				...
				if ( !found )
					throw new ApplicationException("Unknown color space (media subtype)");
			}
			finally
			{
				DsUtils.FreeAMMediaType( mediaType );
		...
		private void setMediaSubType( IAMStreamConfig streamConfig, ColorSpaceEnum newValue )
		{
			if ( videoStreamConfig == null )
				throw new NotSupportedException();
			assertStopped();
		...
		/// <summary>
		///  Utility method for retrieving the current 
		///  brightness/contrast/hue/saturation. This method translates
		///  the property's min/max range to 0% to 100%.
		/// </summary>
		private double getProcAmpProperty( VideoProcAmpProperty prop )
		{
			int i, min, max, step, def, flags;
			int hr;
			hr = videoProcAmp.GetRange( prop, out min, out max, out step, out def, out flags );
			if ( hr < 0 ) Marshal.ThrowExceptionForHR( hr );
			hr = videoProcAmp.Get( prop, out i, out flags );
			if ( hr < 0 ) Marshal.ThrowExceptionForHR( hr );
			return ((i-min*1.0)/(max-min));
		}

		/// <summary>
		///  Utility method for setting the current 
		///  brightness/contrast/hue/saturation. This method translates
		///  the property's min/max range to 0% to 100%.
		/// </summary>
		private void setProcAmpProperty( VideoProcAmpProperty prop, double value )
		{
			int i, min, max, step, def, flags;
			int hr;
			hr = videoProcAmp.GetRange( prop, out min, out max, out step, out def, out flags );
			...
			{
				i = (int)((value * (max-min)) + min);
				i = (int)(Math.Round(i / step * 1.0, 0) * step);
			}
			hr = videoProcAmp.Set( prop, i, 0);
			if ( hr < 0 ) Marshal.ThrowExceptionForHR( hr );
		}
